...
function validerFonctionne(elementId){
	...
	var f3=validerArret('arret1_stop','arret1_start');
	var result;
	if(f1 && f2 && f3){
		result=true;
	}else{
...
function validerArret(idStop,idStart){
	...
	if(result1){
		if(stopVal!=0 && startVal==0){
			//changerSpan(spanId,true,'START vaut 0 mais le temps STOP contient une VALEUR');
			myFunction('span2','holaas',false);
			return false;
		}else{
				return true;}		
	}else{
		//	changerSpan(spanId,true,'le temps STOP doit etre SUPERIEUR au temps de START');
		myFunction('span2','holaas2222',false);
				return false;
	}
	}


function changerSpan(spanId,isShown,message){
	var spantip=document.getElementById(spanId);
	if(isShown){
		//spantip.className='correct';
		spantip.inneHTML=message;
	}else{
		spantip.inneHTML='hOLA';
	}
	}
...
myFunction('span2','holaas',false);
/*
function changerSpan1(spanid,isBon){
	var spantip=document.getElementById(spanid);
	if(isBon){
